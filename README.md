...
Hot Ticket — сервис поиска и отслеживания авиабилетов из Ташкента. Telegram-бот доставляет уведомления и предоставляет быстрые действия, Mini App показывает Hot Deals, Watchlist и историю цен, а административная панель — продуктовую статистику.
...
- фиксированный вылет из `Ташкент (TAS)` и цены в `UZS`;
...
Текущие продуктовые ограничения: origin зафиксирован как `TAS`, валюта — `UZS`, а каталог строится по Aviasales Explore Hot Offers, а не по live-поиску всех авиакомпаний.
...
Команда создаёт `TAS/UZS`, запрашивает Explore API только с
`origin=TAS&currency=uzs` и сохраняет полный каталог. Класс и багаж
...
2. Введите `Стамбул`/`IST` либо нажмите `🌍 Все направления из Ташкента`.
3. Используйте `➡️ Показать ещё`, чтобы пройти весь подходящий каталог.
4. Откройте `/settings`, выберите `Эконом`/`Бизнес` и требование багажа.
5. Создайте тестовое уведомление. Origin и currency бот не спрашивает: они всегда `TAS/UZS`.
