...
- фиксированный каталог рейсов из `Ташкент (TAS)` с ценами только в `UZS`;
- поиск направления по русскому названию или IATA-коду;
- постраничный просмотр всех подходящих билетов по 10 предложений;
- настройки класса перелёта и требования багажа;
- подписки на направления, даты, цену и прямой рейс;
- максимум 20 активных подписок на пользователя;
- единственный активный sync source `TAS/UZS`;
- история цен, price-drop уведомления и защита от повторной отправки;
- SQLite-кэш городов, обновляемый при каждом sync;
