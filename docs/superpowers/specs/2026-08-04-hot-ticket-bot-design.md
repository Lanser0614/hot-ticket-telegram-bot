...
В MVP реализуется транспортно-независимый контракт `POST /internal/jobs/sync-hot-tickets` с bearer-авторизацией. Публикация этого контракта на конкретном бесплатном HTTP-хостинге откладывается до выбора поддерживаемого механизма. Сценарий синхронизации имеет стабильный программный интерфейс, поэтому hosting-адаптер не изменяет бизнес-логику.
...
Базовый адрес хранится в единственной настройке `AVIASALES_EXPLORE_BASE_URL=https://explore-api.aviasales.com` и не дублируется в клиентах. URL builder проверяет два обязательных кода, нормализует регистр и добавляет параметры по умолчанию из ТЗ. Запрос с отсутствующим или некорректным origin/currency не отправляется.
...
Синхронизация разделена на два уровня. `SyncHotTicketsJob.execute()` загружает все активные записи `sync_sources` и по очереди вызывает `SyncTicketsService.execute(input)` для каждой пары. Ошибка одной пары логируется и не останавливает остальные источники.

`SyncTicketsService.execute(input)` обрабатывает ровно одну пару origin/currency:
...
Конкурирующий запуск возвращает или записывает результат `skipped`. Upsert билетов и уникальность уведомлений обеспечивают второй уровень идемпотентности, если инициатор повторит запрос после неопределённого результата. Job возвращает число успешно обработанных источников для безопасного ответа trigger.

Endpoint-контракт принимает только `POST`, проверяет `Authorization: Bearer <SYNC_SECRET>`, запускает `SyncHotTicketsJob` без произвольного origin, currency или внешнего URL и при успехе возвращает только `{ "status": "success", "processed_sources": number }`. Полный ответ Aviasales и внутренние ошибки наружу не передаются. Конкретный hosting-адаптер остаётся отдельным инфраструктурным модулем.
...
Публичный sync endpoint должен использовать HTTPS, принимать только `POST`, проверять bearer secret, не принимать произвольный внешний URL и не возвращать полный ответ Aviasales или внутренние детали ошибок.
...
9. Транспортно-независимый защищённый endpoint-контракт; hosting-адаптер после выбора способа размещения.
